...
Decompositions rendered live in the viewer (solid = fitted primitives, wireframe =
source mesh). Each colour is one primitive; note how the vocabulary mixes types so
...
Press **S** in the viewer to drop a new PNG into `img/` (the collection auto-increments).
