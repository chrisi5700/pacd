...
void write_chunk(std::vector<std::uint8_t>& out, std::string_view type, const std::vector<std::uint8_t>& data)
{
...
Image::Image(int width, int height)
	: Image(width, height, Vec3{0.0F, 0.0F, 0.0F})
{
}
...
{
	const auto pixel_count = static_cast<std::size_t>(m_width) * static_cast<std::size_t>(m_height);
	m_pixels.resize(pixel_count * CHANNELS);
	const std::array<std::uint8_t, CHANNELS> rgb{to_byte(fill_color.x), to_byte(fill_color.y), to_byte(fill_color.z)};
...
Vec3 Image::pixel(int x, int y) const noexcept
{
	...
	return {static_cast<float>(m_pixels.at(base + 0)) * INV, static_cast<float>(m_pixels.at(base + 1)) * INV,
