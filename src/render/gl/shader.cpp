...
std::expected<void, std::string> ShaderProgram::build(std::string_view vertex_source,
    ...
    const std::expected<GLuint, std::string> fragment =
        compile(GL_FRAGMENT_SHADER, fragment_source);
    if (!fragment) {
