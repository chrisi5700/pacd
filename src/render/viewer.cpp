...
Viewer::Viewer(ViewerOptions options) : m_options(std::move(options)) {
    ...
    GLFWwindow* window = glfwCreateWindow(m_options.width, m_options.height,
...
void Viewer::render_frame() {
    ...
    const float aspect =
        static_cast<float>(std::max(m_fb_width, 1)) / static_cast<float>(std::max(m_fb_height, 1));
    m_shader.use();
...
void Viewer::handle_cursor(double xpos, double ypos) {
    ...
    m_last_y = ypos;
    if (m_orbit_active) {
        m_camera.orbit(static_cast<float>(delta_x) * ORBIT_SPEED,
                       static_cast<float>(-delta_y) * ORBIT_SPEED);
