...
{
	return static_cast<std::uint32_t>(data.at(offset)) | (static_cast<std::uint32_t>(data.at(offset + 1)) << 8U) |
...
[[nodiscard]] bool looks_binary(const Bytes& data) noexcept
{
	...
	const std::size_t	expected = BINARY_HEADER + 4 + (static_cast<std::size_t>(count) * BINARY_TRIANGLE);
