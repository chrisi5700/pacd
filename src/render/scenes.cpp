...
const std::vector<Vec3>& palette() {
    static const std::vector<Vec3> COLORS{
        {0.86F, 0.36F, 0.31F}, {0.34F, 0.64F, 0.86F}, {0.46F, 0.80F, 0.46F},
        {0.91F, 0.76F, 0.35F}, {0.70F, 0.51F, 0.86F}, {0.86F, 0.56F, 0.35F},
        {0.42F, 0.80F, 0.80F}, {0.87F, 0.46F, 0.71F},
    };
    return COLORS;
}
...
Scene make_primitive_gallery() {
    ...
    scene.objects.push_back(
        make_object(make_box({1.0F, 1.0F, 1.0F}), {-3.0F, 0.5F, 0.0F}, colors.at(0)));
    scene.objects.push_back(make_object(make_uv_sphere(0.5F), {-1.0F, 0.5F, 0.0F}, colors.at(1)));
    scene.objects.push_back(
        make_object(make_cylinder(0.5F, 1.0F), {1.0F, 0.5F, 0.0F}, colors.at(2)));
