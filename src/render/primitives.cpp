...
Mesh make_box(Vec3 size)
{
	...
	Mesh		mesh;
	add_quad(mesh, {Vec3{hx, -hy, -hz}, {hx, hy, -hz}, {hx, hy, hz}, {hx, -hy, hz}}, {1.0F, 0.0F, 0.0F});
	add_quad(mesh, {Vec3{-hx, -hy, hz}, {-hx, hy, hz}, {-hx, hy, -hz}, {-hx, -hy, -hz}}, {-1.0F, 0.0F, 0.0F});
	add_quad(mesh, {Vec3{-hx, hy, hz}, {hx, hy, hz}, {hx, hy, -hz}, {-hx, hy, -hz}}, {0.0F, 1.0F, 0.0F});
	add_quad(mesh, {Vec3{-hx, -hy, -hz}, {hx, -hy, -hz}, {hx, -hy, hz}, {-hx, -hy, hz}}, {0.0F, -1.0F, 0.0F});
	add_quad(mesh, {Vec3{-hx, -hy, hz}, {hx, -hy, hz}, {hx, hy, hz}, {-hx, hy, hz}}, {0.0F, 0.0F, 1.0F});
	add_quad(mesh, {Vec3{hx, -hy, -hz}, {-hx, -hy, -hz}, {-hx, hy, -hz}, {hx, hy, -hz}}, {0.0F, 0.0F, -1.0F});
	return mesh;
}
...
Mesh make_plane(float width, float depth)
{
	...
	Mesh		mesh;
	add_quad(mesh,
			 {Vec3{-half_w, 0.0F, half_d}, {half_w, 0.0F, half_d}, {half_w, 0.0F, -half_d}, {-half_w, 0.0F, -half_d}},
			 {0.0F, 1.0F, 0.0F});
	return mesh;
}
...
Mesh make_uv_sphere(float radius, int segments, int rings)
{
	...
	for (int ring = 0; ring <= ring_count; ++ring)
	{
		...
		{
			const float theta = (2.0F * PI_F) * (static_cast<float>(seg) / static_cast<float>(seg_count));
...
Mesh make_cylinder(float radius, float height, int segments)
{
	...
	for (int seg = 0; seg < seg_count; ++seg)
	{
		const float theta0 = (2.0F * PI_F) * (static_cast<float>(seg) / static_cast<float>(seg_count));
		const float theta1 = (2.0F * PI_F) * (static_cast<float>(seg + 1) / static_cast<float>(seg_count));
