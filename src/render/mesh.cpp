...
float Aabb::radius() const noexcept
{
...
void Mesh::recompute_smooth_normals()
{
	...
	for (const IndexTriangle& tri : triangles)
	{
		...
		const Vec3 face = cross(positions.at(i1) - positions.at(i0), positions.at(i2) - positions.at(i0));
...
[[nodiscard]] WeldResult weld_indexed(const Mesh& soup, float epsilon)
{
	...
	{ return {std::llround(point.x * inv_eps), std::llround(point.y * inv_eps), std::llround(point.z * inv_eps)}; };
	...
	for (const IndexTriangle& tri : soup.triangles)
	{
		...
		const auto i1 = static_cast<std::size_t>(tri.at(1));
		const auto i2 = static_cast<std::size_t>(tri.at(2));
		if (i0 >= soup.positions.size() || i1 >= soup.positions.size() || i2 >= soup.positions.size())
		{
			continue;
		}
		out.triangles.push_back(
			IndexTriangle{intern(soup.positions.at(i0)), intern(soup.positions.at(i1)), intern(soup.positions.at(i2))});
	}
	return out;
}
