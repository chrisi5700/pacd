...
void config_panel(solver::SolverConfig& config, std::string_view file, std::size_t which,
                  std::size_t total, const RunResult& last, PanelRequest& request) {
    ...
    const std::string status =
        last.ok ? fmt::format("{} primitives  -  {} ms", last.count, last.ms) : "no run yet";
...
[[nodiscard]] RunResult load_into_viewer(Viewer& viewer, const std::string& path, bool flat,
                                         const solver::SolverConfig& config, std::size_t which,
                                         std::size_t total) {
    ...
    const RunResult result{.count = parts.size(),
                           .ms = static_cast<long long>(took.count()),
                           .ok = true};
...
void run_files(Viewer& viewer, const Args& args, solver::SolverConfig& config) {
    ...
    auto load_index = [&](std::size_t which) {
        index = which;
        last_run = load_into_viewer(viewer, args.files.at(which), args.flat, config, which,
                                    args.files.size());
    };
    ...
    viewer.set_on_gui([&] {
        config_panel(config, args.files.at(index), index, args.files.size(), last_run, request);
    });
    viewer.set_on_cycle(advance);
    viewer.set_on_redecompose([&] { load_index(index); });
    viewer.set_on_toggle_original([&viewer] {
...
int main(int argc, char** argv) {
    ...
    if (args.frames > 0 || !args.screenshot.empty()) {
        if (has_files) {
            std::ignore = load_into_viewer(viewer, args.files.at(0), args.flat, config, 0,
                                           args.files.size());
        } else {
